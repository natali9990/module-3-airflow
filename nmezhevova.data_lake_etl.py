...
for i,j in insert_dict.items():
    if i!='dm_user_traffic':
        name_task='ods_'+i
        inquiry=f'insert overwrite table nmezhevova.ods_{i} partition (year='+'{{ execution_date.year }})'+f' select {j[0]} from nmezhevova.stg_{i} where year({j[1]}) = '+ '{{ execution_date.year }};'
